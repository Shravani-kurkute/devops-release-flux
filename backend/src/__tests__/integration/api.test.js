...
describe('POST /api/v1/sdk/evaluate', () => {
  test('TC-009: no API key returns 401', async () => {
    const res = await request(app)
  ...
  test('TC-010: wrong key format returns 401', async () => {
    const res = await request(app)
      .post('/api/v1/sdk/evaluate')
...
describe('POST /api/v1/flags/:flagId/schedule', () => {
  test('TC-011: no token returns 401', async () => {
    const res = await request(app)
      .post('/api/v1/flags/flag-1/schedule')
      .send({ targetStatus: true, scheduledTime: new Date().toISOString() });
    expect(res.status).toBe(401);
  });
});